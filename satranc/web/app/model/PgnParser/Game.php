...
public function getPlayersPrettyPrint()
{
 $players = null;
 if ($this->black && $this->white) {
   if (strpos($this->black, $this->white) !== false) {
     // Event contains site.
     $players = $this->black;
   } else {
     $players = "$this->black vs $this->white";
   }
 } elseif ($this->black) {
   $players = $this->black;
 } elseif ($this->white) {
...
public function getEventSitePlayersPrettyPrint()
{
  $eventSite = $this->getEventSitePrettyPrint();
  $players = $this->getPlayersPrettyPrint();

  if ($eventSite && $players) {
	  return "$eventSite: $players";
  } elseif ($eventSite) {
	  return $eventSite;
  } elseif ($players) {
	  return $players;
